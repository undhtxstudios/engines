...
function spawnBox(
  ...
  size = 1,
  color = WHITE,
  type = box2dBodyTypeDynamic,
  applyTexture = true,
  angle = 0
) {
  size = typeof size == "number" ? vec2(size) : size; // square
  const o = new Box2dObject(
    pos,
    size,
    applyTexture && tile(3),
    angle,
...
function spawnRandomObject(
  ...
  if (randInt(2)) {
    const size = vec2(scale * rand(0.5, 1), scale * rand(0.5, 1));
    return spawnBox(pos, size, randColor(), type, true, angle);
  } else {
    const diameter = scale * rand(0.5, 1);
...
function spawnPyramid(pos, count) {
  for (let i = count + 1; i--; )
    for (let j = i; j--; )
      spawnBox(pos.add(vec2(j - i / 2 + 0.5, count - i + 0.5)), 1, randColor());
}

function spawnDominoes(pos, count, size = vec2(0.5, 2)) {
  for (let i = count; i--; )
    spawnBox(pos.add(vec2(i * size.y * 0.9, size.y / 2)), size, randColor());
}
...
function spawnRope(
  ...
  for (let i = 0; i < count; ++i) {
    const pos = startPos.add(size.multiply(vec2(0, i + 0.5)).rotate(-angle));
...
class CarObject extends Box2dObject {
  constructor(pos) {
...

    {
      ...
      makeWheel(pos.add(vec2(1, -0.6)));
      makeWheel(pos.add(vec2(-1, -0.65)), true);
    }
  }
  applyMotorInput(input) {
    const maxSpeed = 40;
    const brakeAmount = 0.8;
...
class PulleyJointObjects extends Box2dObject {
  ...
  render() {
    ...
    drawLine(topPos, this.connectionPos, 0.2, BLACK);
    super.render();
  }
}
  ...
  constructor(pos, scale, sizeCount, color, maxJointStress = 10) {
    ...
    for (let y = sizeCount.y; y--; )
      for (let x = sizeCount.y; x--; ) {
        ...
        const center = vec2(x - nodeSize.x / 2, y - nodeSize.y / 2);
        const p = pos.add(center.multiply(spacing));
        const o = new Box2dObject(p, vec2(0.4), tile(0), 0, color);
        o.addCircle(objectDiameter);
        o.setFilterData(2, 2);
...
function explosion(pos, radius = 3, strength = 300) {
  ...
  new ParticleEmitter(
    ...
    tile(1), // tileInfo
    hsl(0, 1, 0.5),
    hsl(0.15, 1, 0.5), // colorStartA, colorStartB
    hsl(0, 1, 0.5, 0),
    hsl(0.1, 1, 0.5, 0), // colorEndA, colorEndB
    0.7,
    1,
    0.5,
    0.1,
