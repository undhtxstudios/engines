...
function gameUpdate() {
  ...
  if (mouseWasReleased(0)) {
    // release object
    sound_click.play(mousePos, 1, 0.5);
    if (mouseJoint) {
  ...
  if (keyWasPressed("KeyR")) loadScene(scene); // reset scene
...
function gameRender() {
  ...
  drawRect(vec2(20, 8), vec2(100), hsl(0, 0, 0.8), 0, 0);
  ...
  if (scene == 5) {
    ...
    for (let i = count; i--; ) {
      ...
      drawLine(start, result ? result.point : end, 0.1, color);
    }
  }
}
...
function gameRenderPost() {
  if (mouseJoint) {
    ...
    drawTile(ab, vec2(0.3), tile(0), BLACK);
    drawLine(mousePos, ab, 0.1, BLACK);
  }
  ...
  function drawText(text, size = 40, gap = 50) {
    drawTextScreen(text, pos, size, WHITE, size * 0.1);
    pos.y += gap;
  }
}
